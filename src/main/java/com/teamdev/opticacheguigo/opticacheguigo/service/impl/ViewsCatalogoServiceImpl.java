...
public class ViewsCatalogoServiceImpl implements ViewsCatalogoService {
    ...
    public ModelAndView actualizaMica(AuthHeader userSesion, MicaDto mica) {
        ...
        vistaRegistroPro.addObject("codOperacion", responseAlta.getCodigo())
                ...
                .addObject("materiales", inventarioService.getMateriales(userSesion, 1));
    ...
    @Override
    public ModelAndView registroBaseMaterial(AuthHeader userSession, MaterialDto producto, String view) {
        ModelAndView vistaRegistroPro = new ModelAndView(view);
        producto.setNombre(producto.getNombre().toUpperCase());
        ResponseGeneric responseAlta = catalogoService.registrarBaseMaterial(producto, userSession);
        vistaRegistroPro.addObject("codOperacion", responseAlta.getCodigo())
                .addObject("mensaje", responseAlta.getMensaje())
                .addObject("productoError", producto)
                .addObject("productos", inventarioService.getMateriales(userSession, 1));
        return vistaRegistroPro;
    }

    @Override
    public ModelAndView actualizarBaseMaterial(MaterialDto producto, AuthHeader userSession, String view) {
        ModelAndView mv = new ModelAndView(view);
        producto.setNombre(producto.getNombre().toUpperCase());

        ResponseGeneric responseAlta = catalogoService.actualizarBM(producto, userSession);

        mv.addObject("codOperacion", responseAlta.getCodigo())
                .addObject("mensaje", responseAlta.getMensaje())
                .addObject("productoError", new ProductoDto())
                .addObject("productos", inventarioService.getMateriales(userSession, 1))
                .addObject("productosEliminados", inventarioService.getMateriales(userSession, 0));
        return mv;
